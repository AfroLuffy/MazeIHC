...
//si esta entre ese rango de angulos comienza entra al proceso
    if(xyz[1]>=30||xyz[1]<=-30){

    //Si el valor del eje Y es mayor o igual a 30 grados
        if(xyz[1]>=30){
        Serial.print("Hacia atras:"); Serial.println(xyz[1]); //detecta que se va hacia atras
        Serial.write(1); //escribe el numero de esa instruccion establecida en unity
        Serial.flush(); //espera hasta que se envie el dato
        }
      //Si el valor del eje Y es mayor o igual a -30 grados  
        if(xyz[1]<=-30){
        Serial.print("Hacia delante:"); Serial.println(xyz[1]); //detecta que se va hacia delante
        Serial.write(2); //escribe el numero de esa instruccion establecida en unity
        Serial.flush(); //espera hasta que se envie el dato
        }
    }
    //  EL valor del eje X
    if(xyz[2]>=30||xyz[2]<=-30){
    //Si el valor del eje X es mayor o igual a 30 grados 
        if(xyz[2]>=30){
        Serial.print("Hacia Derecha:"); Serial.println(xyz[2]);//detecta que se va hacia derecha
        Serial.write(3);//escribe el numero de esa instruccion establecida en unity
        Serial.flush();//espera hasta que se envie el dato
        }
        if(xyz[2]<=-30){
        Serial.print("Hacia izquierda:"); Serial.println(xyz[2]);//detecta que se va hacia izquierda
        Serial.write(4); //escribe el numero de esa instruccion establecida en unity
        Serial.flush(); //espera hasta que se envie el dato
        }
    }
  }
